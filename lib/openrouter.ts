...
export async function generateIconWithOpenRouter({
  ...
  const styleDescriptions: Record<string, string> = {
    ...
    gradient: "smooth gradient fills, vibrant color transitions, modern gradient mesh style, depth through color",
    line: "clean line art, outline only style, consistent stroke width, minimal internal details, vector line drawing",
    sketch: "hand-drawn sketch style, organic pencil strokes, artistic and loose, sketch-like texture, natural imperfections",
    minimalist: "ultra-minimalist design, absolute simplicity, single shape focus, essential elements only, maximum negative space",
    cartoon: "playful cartoon style, rounded friendly shapes, bold outlines, fun and approachable, animation-ready design",
    isometric: "isometric 3D perspective, 30-degree angles, technical illustration style, precise geometric forms, professional diagram quality",
  };

  const colorDescriptions: Record<string, string> = {
    vibrant: "vibrant and bold colors with high saturation, eye-catching, energetic color palette, RGB primaries",
    pastel: "soft pastel colors, gentle and calming tones, low saturation, light and airy feel, muted palette",
    monochrome: "single color monochrome design, various shades and tints of one hue, elegant simplicity",
...
- Return ONLY valid SVG code starting with <svg> and ending with </svg>`;
        ...
        const variationResponse = await generateOpenRouterCompletion({
          messages: [
            {
              role: "system",
              content: "You are an expert SVG icon designer. Generate clean, valid SVG code variations. Return only the SVG code without explanation.",
            },
            {
              role: "user",
        ...
        varSvgCode = varSvgCode.replace(/```svg\n?/g, "").replace(/```xml\n?/g, "").replace(/```\n?/g, "");
...
function generateFallbackIcon(
  ...
  if (lowerPrompt.includes("heart") || lowerPrompt.includes("love") || lowerPrompt.includes("like")) {
    return `<svg viewBox="0 0 ${size} ${size}" xmlns="http://www.w3.org/2000/svg">
  ...
  if (lowerPrompt.includes("check") || lowerPrompt.includes("success") || lowerPrompt.includes("complete")) {
    return `<svg viewBox="0 0 ${size} ${size}" xmlns="http://www.w3.org/2000/svg">
  ...
  if (lowerPrompt.includes("settings") || lowerPrompt.includes("gear") || lowerPrompt.includes("config")) {
    return `<svg viewBox="0 0 ${size} ${size}" xmlns="http://www.w3.org/2000/svg">
