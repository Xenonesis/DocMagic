...
export function SimpleThemeToggle() {
  ...
  return (
    <button
      onClick={cycleTheme}
      className="group relative w-9 h-9 rounded-lg border border-border/80 hover:border-border flex items-center justify-center bg-background/90 hover:bg-accent/50 shadow-sm transition-all duration-200 hover:shadow-md overflow-hidden"
      aria-label={`Current theme: ${label}. Click to cycle themes`}
    >
      <div className={`transition-all duration-500 ${isChanging ? "scale-0 rotate-180 opacity-0" : "scale-100 rotate-0 opacity-100"}`}>
