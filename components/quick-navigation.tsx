          ...
          {navItems.map((item, index) => (
            <TooltipWithShortcut key={item.href} content={item.tooltip}>
              <Link
                ...
                className={`group flex flex-col items-center p-4 rounded-xl glass-effect border ${item.borderColor} hover:scale-105 transition-all duration-300 animate-fade-in-up focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-offset-2 focus-visible:ring-blue-500`}
                style={{ animationDelay: `${index * 50}ms` }}
              >
                <div
                  className={`w-12 h-12 ${item.gradient} rounded-xl flex items-center justify-center mb-3 group-hover:scale-110 transition-transform`}
                >
