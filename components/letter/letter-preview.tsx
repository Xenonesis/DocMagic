...
export function LetterPreview({ letter }: LetterPreviewProps) {
  ...
  return (
    <div className="p-8 max-w-[800px] mx-auto font-serif bg-white text-black">
      {/* Sender Information */}
      <div className="mb-8">
        {from.name && <p className="mb-1 font-semibold">{from.name}</p>}
        {from.address && (
      ...
      <div className="mb-8">
        {to.name && <p className="mb-1 font-semibold">{to.name}</p>}
        {to.address && <p className="text-sm text-gray-700 whitespace-pre-line">{to.address}</p>}
      </div>
      ...
      {/* Letter Content */}
      <div className="whitespace-pre-line text-gray-800 leading-relaxed">{content}</div>
    </div>
  );
}
