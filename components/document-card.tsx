...
export function DocumentCard({ title, description, icon, href, className }: DocumentCardProps) {
  ...
  return (
    ...
    >
      ...
      <div
        className={cn(
          'absolute inset-0 opacity-0 group-hover:opacity-30 transition-opacity duration-500 rounded-lg',
          theme.gradientClass,
        )}
      ...
      <CardHeader className="relative z-10 pb-3 sm:pb-4 p-4 sm:p-6">
        <div className="relative">
          <div
            className={cn(
              'h-12 w-12 sm:h-14 sm:w-14 rounded-xl flex items-center justify-center text-white mb-3 sm:mb-4 group-hover:scale-110 transition-transform duration-300 relative',
              theme.gradientClass,
            )}
          >
            {icon}
            ...
            <div
              className={cn(
                'absolute inset-0 rounded-xl opacity-0 group-hover:opacity-50 blur-md transition-opacity duration-300',
                theme.gradientClass,
              )}
            ></div>
          </div>
      ...
      <CardContent className="relative z-10 p-4 sm:p-6 pt-0">
        <div className="h-20 sm:h-24 rounded-lg bg-gradient-to-br from-muted/50 to-muted/80 flex items-center justify-center transition-all duration-500 border border-border/50 relative overflow-hidden group-hover:border-2">
          ...
          <div className="flex items-center gap-2 text-muted-foreground group-hover:text-foreground transition-colors relative z-10">
            ...
            <Star
              className="h-3 w-3 sm:h-4 sm:w-4 group-hover:text-blue-500 transition-colors animate-pulse"
      ...
      <CardFooter className="relative z-10 p-4 sm:p-6 pt-0">
        <Button
          asChild
          className={cn(
            'w-full text-white font-semibold hover:scale-105 transition-all duration-300 text-sm sm:text-base h-9 sm:h-10 relative overflow-hidden',
            theme.gradientClass,
            `group-hover:${theme.glowClass}`,
          )}
        >
