...
async function testSupabaseConnection() {
  ...
  const supabaseKey =
    'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImRoZWl1Y2RhdHJncmtvemtzc3JwIiwicm9sZSI6ImFub24iLCJpYXQiOjE3NjA5OTI0NDMsImV4cCI6MjA3NjU2ODQ0M30.YNK_84VRGyVdQtAqW9a716K5mUmLcaeYFzsg3YKwh8E';
  ...
  try {
    ...
    if (authError) {
      ...
      console.log(
        '📊 Current session:',
        authData.session ? 'Active session found' : 'No active session',
      );
    }
    ...
    if (schemaError) {
      ...
      console.log(
        '📋 Available tables:',
        tables?.map((t) => t.table_name).join(', ') || 'None found',
      );
    }
  } catch (error) {
